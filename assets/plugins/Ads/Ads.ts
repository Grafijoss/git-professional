...
class Ads {
  ...
  static getInstance() {
    // solo añade Ads si no existe la instancia 
    if (!Ads.instance) {
  ...
  private initAds() {
    // crea una copia de ads y conserva la original
    this.ads = [...ALL_ADS]; // destrucruring
  }

  // exponemos ub metodo que va a obtener el ad
  getAd() {
    // si se acaban los ads los inicializamos
    if (this.ads.length === 0) {
      this.initAds();
    }
    // pop saca y elimina un valor del arrglo y lo regresa
    return this.ads.pop();
  }
}
